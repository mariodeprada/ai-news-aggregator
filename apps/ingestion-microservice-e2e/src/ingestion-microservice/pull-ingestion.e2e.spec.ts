...
describe('Pull Ingestion E2E', () => {
  ...
  it('Successfully pulling and extracting a new article', async () => {
    ...
    await context.processScheduledPull.execute();
  ...
  it('Skipping articles that have already been pulled', async () => {
    const source = new RssPullSource(
      'source-1',
      new Date(Date.now() - 600000),
      true,
  ...
  it('Handling source structure changes or connection failures', async () => {
    const failingSource = new RssPullSource(
      'source-1',
      new Date(Date.now() - 600000),
      true,
      'https://example.com/rss',
    );
    const healthySource = new RssPullSource(
      'source-2',
      new Date(Date.now() - 600000),
      true,
      'https://example.com/rss-2',
    );
    await context.pullSourceRepository.save(failingSource);
    await context.pullSourceRepository.save(healthySource);

    context.pullSourceExtractor.setErrorForSource(
      'source-1',
  ...
  it('Processing multiple due sources in a single cycle', async () => {
